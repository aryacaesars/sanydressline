// components/frontend/AboutSection.jsx
"use client";
import {useState, useEffect} from "react";
import {CldImage} from "next-cloudinary";
import {motion} from "framer-motion";
import AboutSectionSkeleton
    from "@/components/frontend/skeleton/AboutSectionSkeleton";
...
const AboutSection = () => {
    ...
    useEffect(() => {
        const fetchAboutContent = async () => {
            try {
                ...
                const aboutData = data.find(item => item.ContentID === 14);
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setFade(false);
            setTimeout(() => {
                setCurrentImageIndex((prevIndex) => (prevIndex + 1) % (aboutContent?.Img.length || 1));
                setFade(true);
            }, 500);
        }, 5000);
    ...
    return (
        <section
            ...
            <div className="flex-1 order-2 md:order-1 md:ml-2">
                ...
                >
                    <CldImage
                        src={aboutContent.Img[currentImageIndex]}
                        alt="About Image"
                        width={500}
                        height={600}
                        crop="fill"
